...
function ChangeProfile() {
  ...
  const onSubmit = () => {
    ...
    if (dataSubmit.status || dataSubmit.avatar || dataSubmit.bg) {
      ...
      if (!urlRegex.test(dataSubmit.avatar)) {
        ...
        setIsSubmitting(true);
        axios
          .put('http://127.0.0.1:3030/api/setting_user/', {
  ...
  return (
    <div>
      {data ? profileSettingsComponent() : (
        <div style={{ display: 'flex', justifyContent: 'center', position: 'relative', top: '350px' }}>
          <CircularProgress disableShrink />
        </div>
      )}
      {error && (
        <Alert variant="outlined" severity="error" onClose={() => setError(null)} style={{position: 'relative', top: '-35px'}}>
          {error}
        </Alert>
      )}
    </div>
